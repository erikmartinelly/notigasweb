/* ==========================================================================
   NOTIGAS - MÃ“DULO PRINCIPAL DE NAVEGACIÃ“N,
   FAVICON DINÃMICO POR CATEGORÃA Y MODO REPARTIDOR EN RUTA
   ========================================================================== */
...
/* Modal de confirmaciÃ³n elegante (Reemplazo de confirm()) */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // PURGA AUTOMÃTICA DE CACHÃ‰ LOCAL (Limpia pedidos antiguos del localStorage, no de la BD)
  // verificarGPSObligatorio() eliminada para no causar doble peticiÃ³n y bloquear PC
  ...
  // AUTODETECTAR Y ACTIVAR MODO SEGÃšN ROL REGISTRADO (COMPRADOR VS REPARTIDOR)
  try {
...
/* ABRE EL MODAL DE CONFIGURACIÃ“N MOSTRANDO LA SECCIÃ“N DEL ROL ACTIVO */
function abrirConfiguracionSegunRol() {
  ...
  if (isDriver) {
    ...
    if (titleEl) titleEl.textContent = 'âš™ï¸ MENÃš Repartidor';
    ...
    if (titleEl) titleEl.textContent = 'âš™ï¸ MENÃš';
  }
...
/* ABRE LA FICHA DEL REPARTIDOR EN MODO EDICIÃ“N (DESDE EL MENÃš CONFIG, NO DEL HEADER) */
function abrirFichaRepartidorEdicion() {
  ...
  // Cambiar tÃ­tulo a modo ediciÃ³n
  ...
  if (mode === 'driver') {
    ...
    if (badgeContainer) {
      badgeContainer.innerHTML = `
        <span style="font-size:9.5px; background:rgba(255,109,0,0.2); color:#FF6D00; padding:3px 6px; border-radius:8px; font-weight:900; border:1px solid #FF6D00;">ðŸš› MODO REPARTIDOR</span>
      `;
    }
    ...
    if (badgeContainer) {
      badgeContainer.innerHTML = `
        <span style="font-size:9.5px; background:rgba(2,136,209,0.2); color:#38BDF8; padding:3px 6px; border-radius:8px; font-weight:900; border:1px solid #0288D1;">ðŸ›ï¸ MODO COMPRADOR</span>
      `;
    }
  }
}
...
window.activarMiUbicacionRepartidor = function() {
  if (typeof currentGpsLat !== 'undefined' && typeof currentGpsLng !== 'undefined' && map) {
    ...
    if (typeof showToast === 'function') showToast('ðŸ“ UbicaciÃ³n Centrada', 'El mapa se ha enfocado en tu posiciÃ³n actual.', 'info', 2000);
  } else {
    if (typeof conectarGPSAuto === 'function') {
      conectarGPSAuto(true);
    }
    if (typeof showToast === 'function') showToast('ðŸ“ Buscando GPS', 'Obteniendo tu ubicaciÃ³n actual...', 'info', 2000);
  }
};
...
window.activarSeguirme = function() {
  ...
  if (btn) {
    ...
    btn.innerHTML = 'ðŸŽ¯ SIGUIENDO';
  }

  if (typeof showToast === 'function') showToast('Seguimiento Activado', 'El mapa seguirÃ¡ tus movimientos automÃ¡ticamente.', 'info', 1500);
};
...
window.desactivarSeguirme = function() {
  if (isMapInteractedByUser) {
    ...
    if (btn && btn.innerHTML.includes('SIGUIENDO')) {
      ...
      btn.innerHTML = 'ðŸŽ¯ INICIAR RECORRIDO';
      if (typeof showToast === 'function') showToast('Seguimiento Pausado', 'Modo exploraciÃ³n manual activo.', 'warning', 1500);
    }
  }
};
...
window.pausarRecorridoRepartidor = function() {
  ...
  if (btnFollow) {
    ...
    btnFollow.innerHTML = 'ðŸŽ¯ INICIAR RECORRIDO';
  }
  isMapInteractedByUser = true; // stops auto-panning

  if (typeof showToast === 'function') showToast('Recorrido Pausado', 'Se ocultÃ³ el camiÃ³n y se detuvo la transmisiÃ³n GPS.', 'warning', 2000);
};
...
function obtenerIconoHtmlPorCategoria(catNombre) {
  ...
  } else if (c.includes('papel') || c.includes('cartÃ³n')) {
  ...
  } else if (c.includes('carbÃ³n') || c.includes('leÃ±a')) {
...
/* PURGA AUTOMÃTICA DE BASE DE DATOS LOCAL Y MEMORIA PARA EVITAR COLAPSO */

/* CAMBIO DE FAVICON E ICONO DE PESTAÃ‘A SEGÃšN EL TIPO DE PEDIDO SELECCIONADO */
function actualizarFaviconSegunPedido(categoria, estado = 'pendiente') {
  ...
  if (cat.includes('gas')) {
    ...
    document.title = "ðŸ”¥ Pedido Activo: Garrafa de Gas GLP - NOTIGAS";
    ...
    document.title = "ðŸ§¼ Pedido Activo: Detergentes - NOTIGAS";
    ...
    document.title = "ðŸ’§ Pedido Activo: Agua 20L - NOTIGAS";
    ...
    document.title = "â™»ï¸ Pedido Activo: Chatarra - NOTIGAS";
    ...
    document.title = "ðŸ“„ Pedido Activo: Papel / CartÃ³n - NOTIGAS";
    ...
    document.title = "ðŸŽ Pedido Activo: Frutas / Verduras - NOTIGAS";
  } else if (cat.includes('carbÃ³n') || cat.includes('leÃ±a')) {
    ...
    document.title = "ðŸªµ Pedido Activo: CarbÃ³n / LeÃ±a - NOTIGAS";
  } else {
    ...
    document.title = "ðŸ“¦ Pedido Activo - NOTIGAS";
  }
}

// FunciÃ³n verificarGPSObligatorio eliminada para evitar redundancia y conflictos con conectarGPSAuto en map.js
...
function getActiveUserLocation() {
  ...
  // Fallback al centro actual del mapa si las coordenadas no estÃ¡n fijadas
...
/* PANORÃMICA DE PEDIDOS ACTIVOS */
// FIX W-02: Reemplaza los mockOrders hardcodeados con datos reales de Supabase.

/* ==========================================================================
   NOTIGAS - APLICACIÃ“N PRINCIPAL (CARRITO, GEOLOCALIZACIÃ“N Y NOTIFICACIONES)
   ========================================================================== */
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('./sw.js?v=75')
      ...
      .catch((err) => console.error('âŒ Error Service Worker:', err));
  });
}

// 2. InicializaciÃ³n principal de la aplicaciÃ³n
document.addEventListener('DOMContentLoaded', () => {
    console.log('ðŸš€ NOTIGAS iniciando...');

    // Escuchar a que Auth termine de inicializar y validar la BD (para asegurar rol y ciudad)
    const initSupabaseFeatures = () => {
        console.log('ðŸ”— Auth y BD sincronizados. Iniciando servicios en red...');
        ...
        if (typeof cargarPedidosVecinalesEnVivo === 'function') {
            ...
            // Mecanismo de recuperaciÃ³n (polling backup) en caso de que falle Realtime
            if (typeof iniciarSuscripcionesRealtime === 'function') {
...
// (El handler de error global ya estÃ¡ registrado al inicio del archivo)

// 5. FunciÃ³n de navegaciÃ³n entre vistas
window.navegarA = function(vista) {
    const vistas = ['mapa', 'foro', 'vendedores', 'pedidos'];

    if (!vistas.includes(vista)) {
        console.error('Vista no vÃ¡lida:', vista);
        return;
    }
