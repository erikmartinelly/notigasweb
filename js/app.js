...
// SIMULACIÓN EN VIVO (20 pedidos, 5 camiones moviéndose por 5 minutos)
window.runLiveSimulation = function() {
  if (typeof L === 'undefined' || typeof map === 'undefined') return;
  
  console.log("Iniciando simulación visual 20 pedidos y 5 camiones...");
  let simMarkers = [];
  let truckSims = [];
  
  let baseLat = -17.3895;
  let baseLng = -66.1568;
  if (typeof currentGpsLat !== 'undefined' && typeof currentGpsLng !== 'undefined') {
    baseLat = currentGpsLat;
    baseLng = currentGpsLng;
  }

  // Crear 20 pedidos
  for(let i=0; i<20; i++) {
    let lat = baseLat + (Math.random() - 0.5) * 0.04;
    let lng = baseLng + (Math.random() - 0.5) * 0.04;
    let m = L.marker([lat, lng], { icon: typeof garrafaIcon !== 'undefined' ? garrafaIcon : new L.Icon.Default(), zIndexOffset: 8000 }).addTo(map);
    m.bindPopup(`<div style="text-align:center;"><strong style="color:#FF6D00;">🛒 Pedido Simulado ${i+1}</strong><br>Garrafa GLP 10kg</div>`);
    simMarkers.push(m);
  }
  ...
  for(let i=0; i<5; i++) {
    let lat = baseLat + (Math.random() - 0.5) * 0.04;
    let lng = baseLng + (Math.random() - 0.5) * 0.04;
    let m = L.marker([lat, lng], { icon: typeof truckIcon !== 'undefined' ? truckIcon : new L.Icon.Default(), zIndexOffset: 9000 }).addTo(map);
    m.bindPopup(`<div style="text-align:center;"><strong style="color:#00E676;">🚚 Repartidor Simulado ${i+1}</strong><br>Buscando pedidos...</div>`);
    truckSims.push({ marker: m, lat: lat, lng: lng });
    simMarkers.push(m);
  }
  ...
  setTimeout(() => {
    ...
    console.log("Simulación finalizada y limpiada.");
  }, 300000);
};

// Iniciar simulación cuando el mapa esté listo
let simCheckInterval = setInterval(() => {
  if (typeof L !== 'undefined' && typeof map !== 'undefined' && map !== null) {
    clearInterval(simCheckInterval);
    if (typeof window.runLiveSimulation === 'function') {
      window.runLiveSimulation();
    }
  }
}, 1000);
