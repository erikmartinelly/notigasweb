-- Supabase Advisor lint 0003: cache session/auth helpers once per statement.
-- Authorization semantics remain unchanged; only fixed per-request functions are wrapped in SELECT.
